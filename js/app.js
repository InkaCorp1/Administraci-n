...
async function loadDesembolsosPendientes() {
    ...
    try {
        ...
        // Cargar estado de documentos para mostrar progreso
        const creditoIds = creditosPendientes.map(c => c.id_credito);
        const { data: documentos } = await supabase
            .from('ic_creditos_documentos')
            .select('id_credito, contrato_generado, pagare_generado, tabla_amortizacion_generada, contrato_firmado, pagare_firmado, tabla_amortizacion_firmada, documento_garante_firmado')
        ...
        container.innerHTML = creditosPendientes.map(credito => {
            ...
            const docs = documentos?.find(d => d.id_credito === credito.id_credito) || {};
            ...
            // Calcular progreso
            const docsRequeridos = 3 + (credito.garante ? 1 : 0);
            let docsListos = 0;
            if (docs.contrato_generado || docs.contrato_firmado) docsListos++;
            if (docs.pagare_generado || docs.pagare_firmado) docsListos++;
            if (docs.tabla_amortizacion_generada || docs.tabla_amortizacion_firmada) docsListos++;
            if (credito.garante && docs.documento_garante_firmado) docsListos++;
            
            const porcentajeProgreso = Math.round((docsListos / docsRequeridos) * 100);
            ...
            return `
                <div class="desembolso-card" data-id="${credito.id_credito}">
                    ...
                    <div class="desembolso-info">
                        ...
                        <div class="desembolso-info-item">
                            <span class="desembolso-info-label">Progreso Docs</span>
                            <span class="desembolso-info-value">${docsListos}/${docsRequeridos} (${porcentajeProgreso}%)</span>
                        </div>
                        <div class="desembolso-info-item">
                    ...
                    <div class="desembolso-progress-bar-container" style="height: 4px; background: #e2e8f0; border-radius: 2px; margin: 10px 0; overflow: hidden;">
                        <div style="width: ${porcentajeProgreso}%; height: 100%; background: ${porcentajeProgreso === 100 ? '#10b981' : '#f59e0b'}; transition: width 0.3s ease;"></div>
                    </div>
