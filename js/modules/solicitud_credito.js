...
async function loadPendientesDesembolso() {
    ...
    try {
        ...
        container.innerHTML = creditosPendientes.map(credito => {
            ...
            const contratoOk = docs.contrato_generado || docs.contrato_firmado;
            const pagareOk = docs.pagare_generado || docs.pagare_firmado;
            const tablaOk = docs.tabla_amortizacion_generada || docs.tabla_amortizacion_firmada;
...
async function generarDocumentoContrato(idCredito) {
    try {
        ...
        const declaraciones = [
            ...
            `2.- Como referencia pongo a **${(socio.referencia || 'PERSONA DE CONFIANZA').toUpperCase()}**, declaro que me conoce y puede preguntar por mí llamando al **${socio.telefonoreferencia || 'NÚMERO REGISTRADO'}**.`,
